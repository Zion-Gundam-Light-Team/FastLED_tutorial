...
// Q5 functions
void dynamicRainbow(CRGB *leds, int numLeds, uint8_t *hue, uint8_t speed){ // Pointer =array
    fill_rainbow(leds, numLeds, *hue, speed); // array [0] = 1000, array[1] = 1001
    FastLED.show();
    delay(100);
    *hue += speed;
}
...
// Q7 functions 
// void breathEffect(CRGB *leds, int numLeds, uint8_t frequency, int brightnessLow, int brightnessHigh, unsigned long onDuration, uint8_t hue){
//     uint8_t currentBrightness = beatsin8(frequency, brightnessLow, brightnessHigh); // 60/ frequency(20) = 3s
//     fill_solid(leds, numLeds, CHSV(hue, 255, currentBrightness));
//     FastLED.show();
//     delay(onDuration);
// }
...
void waveEFfects(CRGB *leds, uint8_t *colorIndex, int NUM_LEDS){
    fill_palette(leds, NUM_LEDS, *colorIndex, 5, oceanPalette, 255, LINEARBLEND);
    FastLED.show();
    delay(50);
    *colorIndex += 5;
}
...
//                          brightnessHigh2, onDuration2);
    ...
    // Q7
    // breathEffect(leds_RGB1, NUM_RGB1, frequency3, brightnessLow3, brightnessHigh3, onDuration3, 0);
    // breathEffect(leds_RGB2, NUM_RGB2, 15, brightnessLow3, brightnessHigh3, onDuration3, 85);
