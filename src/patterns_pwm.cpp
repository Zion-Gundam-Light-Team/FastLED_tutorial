...
void initPwm() {
    ...
    uint8_t pwmAddresses[MAX_NUM_PWM] = {PWM_ADDRESS_1, PWM_ADDRESS_2,
                                         PWM_ADDRESS_3, PWM_ADDRESS_4,
                                         PWM_ADDRESS_5};
...
// 散氣口
uint16_t pwmVent(int patternNum, uint16_t MIN_FREQ, uint16_t MAX_FREQ,
...
uint16_t pwmOff() { return 0; }

uint16_t pwmOn(uint16_t brightness) { return brightness; }

void pwmBreathAll(uint16_t pwmBuffer[][16], int freq, uint16_t brightnessLow,
                  uint16_t brightnessHigh) {
...
bool pwmFadeOutAll(uint16_t pwmBuffer[][16], int fadeSpeed,
    ...
    for (int pwmIndex = 0; pwmIndex < ACTUAL_NUM_PWM; pwmIndex++) {
        ...
        if (currentBrightness != 0) allOff = false;
    }
    return allOff;
}

uint16_t pwmFadeIn(unsigned long& currentTime, int fadeSpeed,
...
uint16_t pwmfadeOut1(int fadeSpeed, uint16_t brightnessHigh,
...
uint16_t pwmFlash(uint16_t brightness, int bpm, unsigned long& lastToggle,
                  bool& isOn)  // return value = brightness
...
uint16_t pwmFlashIdle(uint16_t brightness, int speed, uint8_t& flashCount,
...
uint16_t pwmBreathFlash(int freq, int bpm, uint16_t& breath_brightness,
                        bool& isOn, unsigned long& lastUpdate) {
...
uint16_t pwmBreathFlash1(int freq, uint16_t brightnessLow,
                         uint16_t brightnessHigh, int breathFlashSecond,
                         int breathFlashCount, int countInterval,
                         bool& startRun, int stopSecond, bool& isOn) {
...
uint16_t pwmBreathFlashStop(int freq, int bpm, uint16_t& breath_brightness,
                            bool& isOn, unsigned long& lastUpdate) {
    int interval = 60000 / bpm;
...
uint16_t pwmFlashRandom(int bpm, uint16_t brightnessLow,
...
uint16_t pwmHalfBreathFlash(int bpm, uint16_t& currentBrightness, bool& isOn,
                            unsigned long& lastUpdate) {
    int interval = 60000 / bpm;
    if (millis() - lastUpdate >= interval) {
        lastUpdate = millis();
...
uint16_t pwmValcanGun(uint16_t brightness, int speed, uint8_t& flashCount,
    ...
    if (flashCount <
        2 * flashTime)  // 6 because each flash has ON and OFF states
    {
...
uint16_t pwmHalfBreath(int bpm, uint16_t& currentBrightness,
                       unsigned long& lastUpdate) {
    int interval = 60000 / bpm;
    if (millis() - lastUpdate >= interval) {
        lastUpdate = millis();
...
// 呼吸 + 停 呼吸信號燈
uint16_t pwmBreathStop(uint16_t brightnessLow, uint16_t brightnessHigh,
                       int breathSecond, int breathCount, int stopSecond) {
    static unsigned long totalCycleStart = 0;
    unsigned long startTiming = millis();
    unsigned long totalCycle =
        (breathCount * breathSecond +
         stopSecond);  // For accurately calculate breath in different duration,
                       // stopSecond or breathCount
    static uint16_t scaledBrightness;
    ...
    if (startTiming - totalCycleStart < nBreathTimes) {
        unsigned long currentBreathTiming =
            (startTiming - totalCycleStart) % breathSecond;
        unsigned long halfTime = breathSecond / 2;
        uint16_t brightness;
        // Replace beatsin8(), It is difficult to calculate the time ratio
        // between beatsin8() and stopSecond
        if (currentBreathTiming <= halfTime) {
            brightness = map(currentBreathTiming, 0, halfTime, brightnessLow,
                             brightnessHigh);
        } else {
            brightness = map(currentBreathTiming, halfTime, breathSecond,
                             brightnessHigh, brightnessLow);
        }
        scaledBrightness = brightness;
    } else {
        scaledBrightness = brightnessLow;
        // pinMode(pin, OUTPUT);   //clear PIN setting (value) to make sure
        // brightness is correct
    }
    return scaledBrightness;
}

uint16_t* pwmFlashRandomWithChance(uint16_t brightnessLow,
    ...
    for (int channel = 0; channel < 16; channel++) {
        if (millis() - lastUpdateArr[channel] >= interval) {
            ...
            if (random8() < chance) {
                if (!isOnArr[channel]) {
                    uint16_t brightness =
                        random16(brightnessLow, brightnessHigh);
...
std::array<uint16_t, 2> pwmFlashAlternative(uint16_t brightness, int bpm,
...
uint16_t pwm_led_flash_stop(uint16_t brightness, int flashSpeed, int stopSecond,
                            int flashCcount, unsigned long& lastUpdate) {
    static int count = 0;
    static uint16_t scaledBrightness;
    ...
    if (millis() - lastUpdate >= flashSpeed) {
        scaledBrightness =
            map((count % 2 == 0) ? brightness : 0, 0, 255, 0, 4095);
        count++;
        lastUpdate = millis();
    }
    return scaledBrightness;
...
uint16_t pwmFlashStop(uint16_t brightness, int flashSpeed, int stopSecond,
                      int flashCount, unsigned long& lastUpdate) {
    static int count = 0;
...
uint16_t pwmProgressiveFlash(uint16_t brightnessHigh, uint16_t brightnessLow,
...
bool randomLightup(uint16_t pwmBuffer[][16], int maxNumPwm, int slaveId,
    ...
    for (int i = 0; i < maxNumPwm; i++) {
        for (int j = 0; j < 16; j++) pwmBuffer[i][j] = 0;
    }
    if (!isFlashing) {
...
//
uint16_t pwmFlashKeep(uint16_t brightness, float flashCount,
                      int flashDurationMs, int keepOnMillis,
                      unsigned long& lastUpdate) {
